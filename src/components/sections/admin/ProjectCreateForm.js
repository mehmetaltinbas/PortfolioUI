function ProjectCreateForm({ formData, onChange, onSubmit, isHidden }) {
    return (
      <form id="projectCreateForm" onSubmit={onSubmit} className={`${isHidden ? 'hidden' : ''} absolute bg-white p-6 rounded-2xl shadow-md border flex flex-col justify-center items-center gap-2`}>
        <div>
          <label>Title: </label>
